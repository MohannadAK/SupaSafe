...
RUN echo '#!/bin/sh' > /app/start.sh && \
    ...
    echo '  kill -TERM $BACKEND_PID 2>/dev/null || true' >> /app/start.sh && \
    echo '  wait $BACKEND_PID 2>/dev/null || true' >> /app/start.sh && \
    ...
    echo '' >> /app/start.sh && \
    echo 'tail -f /app/logs/backend.log &' >> /app/start.sh && \
